...
const ProfilePage: React.FC<ProfilePageProps> = ({ onNavigate, currentPage }) => {
  ...
  const handleSaveName = () => {
    if (tempName.trim()) {
      ...
      localStorage.setItem('ecosage-userName', tempName.trim());
    }
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        localStorage.setItem('ecosage-profileImage', result);
      };
  ...
  const handleRemoveImage = () => {
    setProfileImage('');
    localStorage.removeItem('ecosage-profileImage');
  };

  const handleClearHistory = () => {
    localStorage.removeItem('ecosage-scanHistory');
    setScanHistory([]);
    setUserStats({
      totalScans: 0,
      carbonSaved: 0,
  ...
  const handleExportData = () => {
    ...
    a.href = url;
    a.download = `ecosage-data-${Date.now()}.json`;
    a.click();
    setShowModal(false);
  };

  const handleShare = () => {
    const text = `I've scanned ${userStats.totalScans} products with EcoSage and saved ${userStats.carbonSaved.toFixed(1)}kg of CO2! 🌱 Join me in making sustainable choices!`;
    if (navigator.share) {
                ...
                <div className="text-center">
                  ...
                  <div className="bg-gray-800/50 rounded-xl p-6 mb-6 text-left">
                    <p className="text-white text-sm leading-relaxed">
                      I've scanned <strong>{userStats.totalScans} products</strong> with EcoSage and saved <strong>{userStats.carbonSaved.toFixed(1)}kg of CO₂</strong>! 🌱 Join me in making sustainable choices!
                    </p>
                  </div>
                  <div className="flex space-x-3">
                    <button
